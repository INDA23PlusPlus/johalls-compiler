...
pub fn main() !void {
    ...
    const file_contents = try input_file.readToEndAlloc(gp_allocator, 100 << 20); // if your file is more than 100MB, wtf are you doing?
    defer gp_allocator.free(file_contents);

    var tokens = try tokenize(gp_allocator, file_contents
    // "\nfn fib(n)"
    ...
    const c_str = try ast.as_c(gp_allocator);
    defer c_str.deinit();
    try stdout.print("{s}\n", .{c_str.items});
}
